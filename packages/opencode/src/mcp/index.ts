...
function createClient(directory: string) {
  const client = new Client({ name: "opencode-mod", version: InstallationVersion }, CLIENT_OPTIONS)
  ...
  Effect.gen(function* () {
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(cmd === "opencode" || cmd === "opencode-mod" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  // fork: FSUtil/Global/RuntimeFlags are required by the Claude-Code MCP injection
  deps: [CrossSpawnSpawner.node, McpAuth.node, EventV2Bridge.node, Config.node, FSUtil.node, Global.node, RuntimeFlags.node],
})
